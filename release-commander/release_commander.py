...
def apply_fixes(repo: Path, results):
    ...
    if "E1" in fails:
        staging = repo / "config" / "staging.env"
        prod = repo / "config" / "prod.env"
        stext = staging.read_text(errors="ignore")
        if "REDIS_URL" not in stext:
            staging.write_text(stext.rstrip() + "\nREDIS_URL=redis://staging-cache:6379\n")
        ptext = prod.read_text(errors="ignore")
        prod.write_text(ptext.replace("LOG_LEVEL=debug", "LOG_LEVEL=info"))
        applied.append("E1 — aligned config keys (added REDIS_URL to staging, fixed prod LOG_LEVEL)")
...
def generate_artifacts(repo: Path, release_ref: str, slug: str):
    ...
    release_notes = REPORTS / f"release-notes-{slug}.md"
    ...
    runbook = REPORTS / f"rollback-runbook-{slug}.md"
    runbook.write_text(
